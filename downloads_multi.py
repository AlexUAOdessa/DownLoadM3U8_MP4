    ...
    try:
        ...
        with open(file_path, 'wb') as file:
            for data in response.iter_content(chunk_size=1024):
                ...
                print(f"\rЗагрузка {file_path}: {downloaded_size / (1024 * 1024):.2f} MB из {total_size / (
                    1024 * 1024):.2f} MB): [{'#' * done}{'.' * (50 - done)}] {percent_done:.2f}%", end='')
...
def convert_segments_to_mp4(segment_files, mp4_file):
    try:
        ...
        command = ['ffmpeg', '-y', '-f', 'concat', '-safe',
                   '0', '-i', 'filelist.txt', '-c', 'copy', mp4_file]
